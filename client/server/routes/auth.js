...
function validateSignupForm(payload) {
  ...
  if (!payload || typeof payload.sex !== 'string' || payload.sex.trim().length === 0) {
    isFormValid = false;
    errors.sex = 'Please provide your gender.';
  }

  if (!payload || typeof payload.pob !== 'string' || payload.pob.trim().length === 0) {
    isFormValid = false;
    errors.pob = 'Please provide your state of Origin.';
  }


  if (!payload || typeof payload.status !== 'string' || payload.status.trim().length === 0) {
    isFormValid = false;
    errors.status = 'Please provide your marital status.';
  }


  if (!payload || typeof payload.status !== 'string' || payload.status.trim().length === 0) {
    isFormValid = false;
    errors.age = 'Please provide your Date of Birth.';
  }
